...
# at least one of the elements of the array returns true when given to the block. 
...
# that return true when given to the block. 
# Solve this using Array#each
...
def filter_out(arr, &prc)
    filter = []
    arr.each {|ele| filter << ele if !prc.call(ele)}
    arr = filter
    arr
end
...
# The method should remove elements of the input array that return true 
...
# and a block as arguments. 
# The method should return a new array where each element of the original array 
# is repeatedly run through the block n times. 
# If the number argument is not passed in, 
...
# The method should return a new array where the elements that 
# return true when given to the block come before all of the elements that 
# return false when given to the block. 
...
def proctition(arr, &prc)
    priority = []
    later = []
    arr.each {|e| if prc.call(e) then priority << e else later << e end}
    priority + later
end
...
# when passed into the first proc with their return values 
# when they are passed into the second proc. 
# This method should mutate the input array and return nil.

def selected_map!(arr, prc1, prc2)
    arr.map! {|e| if prc1.call(e) then prc2.call(e) else e end}
    nil
end
...
# is appended with a suffix if the original word returns true
# when given to the corresponding proc key. 
...
def proc_suffix(sent, prcs)
    appended = sent.split(' ')
    prcs.each do |prc, suff|
...
# any number of additional procs as arguments. 
...
# should contain the elements that return true when passed into the second proc.
# If an element returns true for multiple procs, 
# then it should only be placed into the array that corresponds to the 
# proc that appears first in the arguments.

def proctition_platinum(arr, *prcs)
    proctition = Hash.new([])
    prcs.length.times {|i| proctition[i + 1] = []}
    arr.each do |ele|
...
# is changed by a value proc if the original word returns 
# true when passed into the key proc. If an original word returns true 
# for multiple key procs, 
...
def procipher(sent, prcs)
    ...
    prcs.each do |k, v|
        sent.split(' ').each_with_index do |word, i|
            cipher[i] = v.call(cipher[i]) if k.call(word)
        end
    end
    cipher.join(" ")
end
...
# The hash contains procs as both keys and values. 
# The method should return a new sentence where each word of the input sentence 
# is changed by a value proc if the original word returns true 
